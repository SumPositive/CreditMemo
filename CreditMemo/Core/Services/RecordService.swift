...
enum RecordService {
    ...
    /// 意図せず確定してしまう。失敗時は rollback して未保存変更を破棄してから投げ直す。
    ///
    /// なお rollback が戻すのは context の未保存変更であって、既に登録済みの
    /// インスタンスのプロパティ値そのものではない（record.zName などはメモリ上
    /// 新しい値のまま残る）。編集画面は保存失敗時に閉じず入力内容を保つ仕様なので
    /// これは意図どおりで、DB へ漏れないことが担保できていればよい。
