...
/// タグ選択シート（タグ式）の行数見積もり。
///
/// シート高さは packedRowCount の行数で決まる。この関数は AZFlowLayout(packToFill:)
/// と同じ規則でカプセルを詰める前提なので、ズレると実際の表示より高い／低いシートが
/// 開いてしまう。ビルドは通ってしまうため、規則そのものをここで固定する。
@MainActor
struct TagSelectionDetentsTests {
    /// iPhone 15 相当の幅から、カプセル帯の左右余白を引いた実効幅
    private let contentWidth: CGFloat = 393 - 16 * 2

    @Test("タグが無いときも1行ぶんは確保する")
    func emptyKeepsOneRow() {
        #expect(TagSelectionList.packedRowCount(names: [], availableWidth: contentWidth) == 1)
    }

    @Test("短いタグは1行に複数入る")
    func shortNamesShareRow() {
        let names = ["食費", "日用品", "交通"]
        let rows = TagSelectionList.packedRowCount(names: names, availableWidth: contentWidth)
    ...
    func moreTagsNeedMoreRows() {
        let few = TagSelectionList.packedRowCount(
            names: ["食費", "日用品", "交通"],
            availableWidth: contentWidth
        )
        let many = TagSelectionList.packedRowCount(
    ...
    func narrowerWidthNeedsMoreRows() {
        ...
        let wide = TagSelectionList.packedRowCount(names: names, availableWidth: contentWidth)
        let narrow = TagSelectionList.packedRowCount(names: names, availableWidth: 200)
        #expect(narrow >= wide)
    }

    @Test("1つでは収まらない長いタグ名でも1行として数える")
    func overlongNameStillOccupiesARow() {
        let rows = TagSelectionList.packedRowCount(
    ...
    func fullWidthNamesWrapSooner() {
        ...
        let fullWidthRows = TagSelectionList.packedRowCount(names: names, availableWidth: contentWidth)
        let asciiRows = TagSelectionList.packedRowCount(names: ascii, availableWidth: contentWidth)
    ...
    @Test("少ないタグでは全開以外の高さも選べる")
    func fewTagsOfferCompactHeight() {
