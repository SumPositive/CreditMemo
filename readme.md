...
- **金額テンキーに四則演算を追加**（`NumericKeypadView`）
  - `+` `-` `×` `÷` を追加し、途中式と結果を金額欄の下に表示
  - 乗除算の右辺は通貨額ではなく整数倍率として扱う（`enteredValue`）
  - 割り切れない場合の丸めを選べるようにした（`NumericCalculatorRounding`：切り上げ／四捨五入／切り捨て）。既定は四捨五入で、選択は次回以降も引き継ぐ
  - 丸めが必要なときだけ丸め選択と `(?)` ヘルプを出す（`needsRounding`）。丸め名称は言語で長さが変わるため、幅は内容に合わせた可変とした
- **テンキーを独自 View 化**（`NumericKeypadOverlay`）
  - シートの上下動でタップを取りこぼす問題への対策。アニメーションも除去
- **ラベル入力の候補表示を刷新**（`RecordEditView`）
  - 候補をキーボード直上のカプセル帯で表示。`safeAreaInset` に置き、実レイアウト領域を確保することでキーボードに隠れず、背面の行へタップが抜けない
  - `AZFlowLayout` で折り返して2行表示（`packToFill` で行末の余白に後続候補を繰り上げ）。収まらない分は縦スクロール
  - 絞り込みを **前方一致 → 部分一致** の順に並べ替え。一致部分は太字で示す
  - 一致0件のときに全候補へ戻る挙動をやめ、非表示にした
  - 日本語入力の変換中にタップすると、IME の未確定文字が書き戻されて選択が消える不具合を修正（`pendingUsePointSelection`）
  - 入力中のクリアボタンを追加。画面スクロールでキーボードを閉じるようにした（`.scrollDismissesKeyboard(.immediately)`）
- **決済一覧のタグ絞り込みに OR／AND を追加**（`TagMatchMode`）
  - タグ選択シートのソート欄の上にラジオボタンを配置。既定は OR
  - AND は選択タグをすべて持つ明細だけを対象にする（`isSubset(of:)`）
  - 絞り込み表示は件数（「タグ 2件」）をやめ、タグ名を一致条件の記号（OR は `/`、AND は `&`）で列記する形に変更
- **決済一覧の頭出しが効かなくなっていた不具合を修正**
  - 2.6.2 で入れた「当日に近い明細から表示」が、月計の導入（月グループでのネスト）以降は初期表示で働いていなかった
  - 原因は再試行が `.task(id:)` 内にあり、List の再構築で取り消されて初回1回しか実行されていなかったこと。ビューの生存に依存しないタイマー方式へ変更
  - あわせて、決済セルとコピー仮明細を1つの `@ViewBuilder` で返していた箇所を分割し、`.id()` が単独行に対応するようにした
- **保存後の連続入力でテンキーが自動表示されていた不具合を修正**
  - 「クリアして連続入力する」「同じ日付・手段で続ける」の分岐が、`autoOpenAmountPad` 設定を無視して無条件にテンキーを開いていた

## バージョン 2.6.2 の主な変更

- **引き落とし状況のフィルターにタグを追加**
- **決済一覧・引き落とし状況の表示条件を保持**（`SavedConditions`）
  - アプリ起動中のみ保持し、画面を行き来しても直前の条件を引き継ぐ
- **決済一覧を利用日順にしたとき、当日に近い明細から表示**（`prepareClosestDateScroll`）
- **決済一覧を利用日順にしたとき、月計を表示**
- **バナー広告を主メニュー上部のみに変更**
  - 「クリックが誘導されている」で配信停止された件への対策。表示の ON／OFF 設定も追加（既定 OFF）
- **タグ／決済手段／口座の選択シート高を行数に応じて最大化**
- **タグ一覧シートのデザインを共通部品化**（`TagSelectionList`）
